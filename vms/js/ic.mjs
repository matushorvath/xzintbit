...
const inn = async (s) => run({ ...s, mem: setparam(0, s, await getinput()), ip: s.ip + 2 });
...
const setmem = (m, a, v) => [...m.slice(null, a), v, ...m.slice(a + 1)];

const getinput = async () => new Promise((resolve, reject) => fs.read(
    process.stdin.fd, { buffer: Buffer.alloc(1) }, (e, c, b) => c ? resolve(b[0]) : reject(e)));

console.log(await main(process.argv[2]).catch(e => e));
